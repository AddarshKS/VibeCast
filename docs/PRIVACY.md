...
- Playlist recovery: an account-scoped UserDefaults record contains the Spotify account ID, proposed playlist name/description, verified track metadata, creation time, and either an uncertain creation UUID or the created playlist's ID/URL. It is saved before creation and retained across relaunch, permission reconnect, and Spotify sign-out until verified completion, a definite creation rejection, or an explicit Stop recovery choice. It has no automatic 24-hour expiry. Only the connected account's record is shown or acted on; another account's record remains local and hidden. Stopping recovery removes that account's local record without deleting anything from Spotify. Old single-record storage is migrated into this account-scoped archive.
- To recover an uncertain creation, Spotify's playlist description includes a `[VibeCast:request-UUID]` marker. This identifies the creation attempt and contains no account ID or credentials. The marker remains in Spotify after completion or stopping local recovery unless the user edits the description.
- Recent diagnostic history: memory only, up to eight requests, including success, failure, cancellation, and final stage. Activity log: up to 100 in-memory stage/status/error entries, including selected playlist names and search phrases. Central redaction removes recognized credentials, bearer/basic values, OAuth code/state parameters, API keys, and other credential fields before log display or copying; it also masks email addresses and local user-directory names. Raw provider errors do not appear as ordinary request feedback. Copying the log is an explicit user action; review it before sharing, since music requests and metadata may still be personal.
...
- Disconnect Spotify clears pending recommendations, displayed player/request history, tokens and notifications. It hides recovery UI but deliberately retains the account-scoped playlist recovery record described above, so reconnecting cannot silently create a duplicate. Reconnect that account and finish or explicitly stop recovery to remove its record. Disconnect ChatGPT separately logs out VibeCast's local Codex connection. The optional personal OpenAI key has its own Remove saved key command.
