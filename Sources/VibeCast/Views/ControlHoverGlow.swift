...
struct ControlHoverGlow: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30, paused: reduceMotion)) { context in
            ...
            let strength = reduceMotion ? 0.6 : (1 - cos(phase * 2 * .pi)) / 2
            ...
            if style == .wave {
                ...
                shape.fill(.teal.opacity(0.10 + strength * 0.13))
                    .shadow(color: .teal.opacity(0.25 + strength * 0.3), radius: 4 + strength * 4)
            }
        }
