...
- On the experimental compact-dropdown branch, both dropdown and detached player use the same 340-point compact layout. Only detached Lyrics Mode can be resized manually, with a 344-point minimum. Other screens automatically fit their content. Switching containers preserves the selected panel, Lyrics Mode, miniplayer, Advanced View, and draft. Redocking resets only custom sizes. See [the experiment notes](docs/COMPACT_DROPDOWN_EXPERIMENT.md).
- Click album artwork to enter/exit the artwork-centered miniplayer with a ripple transition. Playback, seeking, lyrics, queue, and Spotify device controls remain available. A cached, locally sampled artwork color adds a subtle translucent tint; no artwork or credentials are sent to AI for this effect.
- Requests live on the landing screen (and in developer view), not on lyrics, queue, device picker, or miniplayer. Lyrics and queue reclaim the composer space and share a fixed reading height. Their content fades between panels while native anchoring remains stable.
